...
description: Verified City reads, parameterized building/species queries, reversible CityData fields и destruction lifecycle.
---
...
|---|---|---|---|
| `countBuildingsOfID(String)->Int32` | public | key `City.buildings_dict_id`: `bonfire` | `1` |
| `getBuildingListOfID(String)->List<Building>` | internal | тот же City dictionary key | count `1` |
| `getBuildingListOfType(String)->List<Building>` | internal | key `City.buildings_dict_type`: `type_bonfire` | count `1` |
| `getSubspeciesId(String)->Int64` | public | key `City._species`: `human` | `2` |
...
Canonical 0.6 fix5 закрепил `CityManager F:dict` и owner `Kingdom F:cities`:
...
Это `VERIFIED-LIFECYCLE` natural path.
